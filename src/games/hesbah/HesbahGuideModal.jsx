...
/** دليل كامل — للمشرف والمتسابق (مثل القميري والألقاب) */
export default function HesbahGuideModal({ onClose }) {
  ...
  const content = (
    <div
      className="hesbah-guide-portal hesbah-theme"
      role="dialog"
      aria-modal="true"
      aria-labelledby="hesbah-guide-title"
      onClick={onClose}
    >
      <div
        className="hesbah-guide-modal"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="hesbah-guide-modal__head">
          <div id="hesbah-guide-title" className="hesbah-guide-modal__title">
            📖 دليل لعبة حَسْبة
          </div>
        ...
        <div className="role-toggle" style={{ marginBottom: 16 }}>
          <button
            type="button"
            className={`role-btn ${guideRole === 'player' ? 'active' : ''}`}
          ...
          <button
            type="button"
            className={`role-btn ${guideRole === 'admin' ? 'active' : ''}`}
        ...
        {guideRole === 'admin' && (
          <>
            ...
            {[
              ...
              ['🕶️', 'ظلام — البطاقات الحية لا تظهر حتى اكتمال الإجابات أو انتهاء الوقت'],
            ].map(([ic, tx], i) => (
              <div key={i} className="rule-row">
                {ic} <span>{tx}</span>
              </div>
        ...
        <button type="button" className="btn bg hesbah-guide-modal__done" onClick={onClose}>
          ✅ فهمت!
        </button>
      </div>
    </div>
