...
export default function PackagePaymentSupport({ message, paymentId, onOpenRefund }) {
  ...
  const waHref = `${SUPPORT_WHATSAPP_URL}?text=${waText}`;

  return (
    <div className="pkg-pay-support" role="alert">
      <p className="pkg-pay-support__msg">{message}</p>
      {paymentId ? (
        <p className="pkg-pay-support__id">
          رقم العملية:
          {' '}
          <button
            type="button"
            className="pkg-pay-support__id-btn"
